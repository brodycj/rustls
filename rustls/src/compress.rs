...
pub struct CompressionCacheInner {
    ...
    entries: Mutex<VecDeque<ArcAlias<CompressionCacheEntry>>>,
}
...
impl CompressionCache {
    ...
    ) -> Result<ArcAlias<CompressionCacheEntry>, CompressionFailed> {
        match self {
    ...
    fn compression_for_impl(
        ...
        for (i, item) in cache.iter().enumerate() {
            if item.algorithm == algorithm && item.original == encoding {
                ...
                cache.push_back(ArcAlias::clone(&item));
                return Ok(item);
            }
        }
        drop(cache);
        ...
        let new_entry = ArcAlias::new(CompressionCacheEntry {
            algorithm,
            original: encoding,
        ...
        cache.push_back(ArcAlias::clone(&new_entry));
        Ok(new_entry)
    }
    ...
    fn uncached_compression(
        ...
        Ok(ArcAlias::new(CompressionCacheEntry {
            algorithm,
            original: Vec::new(),
            compressed: CompressedCertificatePayload {
                alg: algorithm,
